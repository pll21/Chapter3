...
datapath <- "Data/GBM/GBMout"
colguide <- c("character", "NULL", "numeric")

inputFiles <- list.files(datapath, pattern = "demo")
inputFiles
groupNames <- c("Full Analysis",
                  "KSEA substrates\n only",
                  "Predictions only")
...
merge_Filter <- merge_Filter[rowSums((merge_Filter > 95 | merge_Filter < 5))>=1, ]
...
pdf(file.path(heatmapfolder, "pKSEA_GBM.pdf"),    # create PNG for the heat map
...
# creates a 5 x 5 inch image
pdf(file.path(heatmapfolder, "pKSEA_GBM_filtered.pdf"),    # create PNG for the heat map
    width = 10,        # 5 x 300 pixels
    height = 12,          # 300 pixels per inch
    pointsize = 9)        # smaller font size
