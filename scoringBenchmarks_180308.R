...
# load("predonly_permscores.RData")
...
counts <- count(kincononly_df, vars = "Condition")
...
calcauc <- function(df){
  auc <- sum(df$TPR*diff(c(0,df$FPR)))
  return(auc)
}

calcpres <- function(df){
  pres <- df$pres[which.max(df$TPR[df$TPR <0.5])]
  return(pres)
}
...
resultaucs <- as.matrix(lapply(permscores_calc, calcauc))
resultaucs <- unlist(resultaucs)
resultaucs_CI <- c(median(resultaucs) - sd(resultaucs)*1.96, median(resultaucs) + sd(resultaucs)*1.96)

resultpres <- as.matrix(lapply(permscores_calc, calcpres))
resultpres <- unlist(resultpres)
resultpres_CI <- c(median(resultpres) - sd(resultpres)*1.96, median(resultpres) + sd(resultpres)*1.96)
