...
inputFiles <- list.files(datapath, pattern = "full full")
inputFiles
inputFiles <- inputFiles[c(4,
                           3,
                           1,
                           2,
                           5)]
inputFiles
groupNames <- c("Dasatinib",
                  "\nCombo/Rap\n(Das contribution)",
                  "Combination",
                  "\nCombo/Das\n(Rap contribution)",
                  "Rapamycin")
...
merge_Filter <- merge_Filter[rowSums((merge_Filter > 95 | merge_Filter < 5))>=2, ]
...
pdf(file.path(heatmapfolder, "pKSEA_crosscompare.pdf"),    # create PNG for the heat map
...
# creates a 5 x 5 inch image
pdf(file.path(heatmapfolder, "pKSEA_crosscompare_filtered.pdf"),    # create PNG for the heat map
    width = 10,        # 5 x 300 pixels
    height = 12,          # 300 pixels per inch
    pointsize = 9)        # smaller font size
